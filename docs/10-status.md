...
1. **M6 — Сделка:** чат WS (Socket.IO `/chat`, read-статусы Ч-*), лента уведомлений S-32 + бейдж (НФ-06), завершение заказа с таймерами 24 ч (ЗВ-02/ЗВ-03/ЗВ-04, cron автозакрытия), отзывы 1–5★ + стоп-словарь + жалобы (О-*, ОМ-*), экраны S-25…S-27, S-30, S-32, S-33.

Дальше по цепочке M7 → M8 без пауз на подтверждение (правило работы №1 из ZOVU_PROMPT.md §11).
