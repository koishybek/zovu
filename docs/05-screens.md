...
| S-14 | Отклики специалиста | `/sp/bids?tab=active\|pending\|archive` | таб 3 |
...
### S-14 Отклики — `/sp/bids?tab=active|pending|archive` (таб 3)
- **Содержимое:** табы «В работе / Ожидают / Архив» (группировка по стадии, G4 п.12): «В работе» = принятый отклик на живом заказе (in_progress/awaiting_confirmation/disputed — во время спора чат остаётся доступен), «Ожидают» = pending, «Архив» = not_selected/declined и завершённые. Карточки со статус-чипами «Ожидание ответа» (warning), «Принят» (success), «Не выбран» (inkSecondary `#6B7280` на divider `#F0F1F4`) — палитра чипов в [06-design-system.md](06-design-system.md). Карточка нажимаема только у активной сделки (ведёт в чат); прочие — статичные.
...
| Каскад «Не выбран» (not_selected) | специалист | S-14 `/sp/bids?tab=archive` |
