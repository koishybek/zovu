...
| **3 — Доверие и безопасность** | 🟡 частично: **3A** ✅ — safety-слой на активном заказе (`SafetySheet`: поделиться статусом через Web Share, SOS·112, «Сообщить о проблеме», доверенные контакты), двухшаговый условный отзыв S-27 (звезда → 4–5★ хвалебные теги/публично, 1–3★ приватная форма проблемы → поддержка), маскирование контактов в тексте отклика (backend `maskContacts` — телефоны/ссылки/@ники/мессенджеры → •••). Отложено: №19 «кто посмотрел/откликнулся» (нужен трекинг просмотров). Чат уже гейтится (создаётся только после принятия). |
