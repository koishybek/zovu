...
Возможные follow-up (не входят в скоуп MVP): карта OSM/Leaflet (S-10/S-22), нативный
порт на Flutter поверх того же API (ADR-008), прод-адаптеры интеграций (Mobizon/Kaspi/
FCM/Anthropic — сейчас заглушки), нативная вычитка kk-переводов.
...
|---|---|---|
| **Карта (S-10/S-22) — заглушка** | Вид «Карта» в сегменте специалиста и карта заказчика — информативная заглушка, не интерактивный Leaflet/OSM. | Лента-колода и список (§4.3) + seed-данные полностью покрывают демо happy-path; интеграция Leaflet (пакет + тайлы + маркеры + CSP PWA) — follow-up. Гео-выдача (расстояние/фильтры) на бэке уже работает. |
