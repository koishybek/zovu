...
**Следующий этап — UX-полировка до тест-юзеров:** см. [11-ux-roadmap.md](11-ux-roadmap.md)
(5 спринтов по бенчмарк-исследованию 13 приложений; платежи вне скоупа). Ожидает
утверждения приоритетов заказчиком.
...
| 2026-07-07 | **UX-роадмап создан** ([11-ux-roadmap.md](11-ux-roadmap.md)) — по бенчмарк-исследованию 13 приложений (inDrive/YouDo/Profi/Яндекс.Услуги/TaskRabbit/Thumbtack/Uber-Bolt/Kaspi + 5 UX-измерений). 28 пунктов в 5 спринтах до готовности к тест-юзерам; платежи вне скоупа. Ожидает утверждения приоритетов. |
| 2026-07-09 | **G4 «Мои активные по статусам» реализован** (спека полноты, п.12 ТЗ). Плоский список откликов специалиста (S-14) сгруппирован в 3 таба: **В работе** (принятый отклик на живом заказе in_progress/awaiting_confirmation/disputed) / **Ожидают** (pending) / **Архив** (not_selected/declined/завершённые), с пустым состоянием на вкладку. Убран «обманчивый pressable» — карточка нажимаема только у активной сделки (→ чат). Уважается `?tab=` из URL (и при уже открытом экране — через effect), диплинк push «Не выбран» → `/sp/bids?tab=archive`. Лёгкий adversarial-ревью: критичных багов нет; починены 2 находки — спорная (disputed) сделка не выпадает из «В работе» и сохраняет доступ к чату; `?tab=` переключает таб и когда экран уже открыт. Build web+api зелёный, 36 jest, вики (05-screens) синхронизирована, проверено скриншотами (3 таба + honor ?tab). |
| 2026-07-09 | **G3 «Тред поддержки + флоу жалобы» реализован** (спека полноты, п.15/16 ТЗ). **Поддержка как полноценный тред** (`SupportTicketScreen`, S-31a): карточки обращений теперь открываются, сообщения пузырями пользователь/поддержка, ответ (`addTicketMessage`), оценка поддержки после закрытия (`rateTicket`, СП-10); на `resolved` инпут заменён на «Решено» (сообщения не уходят в никуда). **Многошаговая жалоба** (`ReportFlow`, ОМ-03): причина (обман/грубость/не пришёл/спам/другое) → комментарий → подтверждение отправки. Точки входа: «Пожаловаться» на отзыве, флаг в чате, «Сообщить о проблеме» в safety-щите. **Ключевое решение:** жалоба НЕ передаёт `orderId` — чтобы не диспьютить заказ (диспьют только через явную отмену G2). Категории тикета локализованы (`labels.ts` — kk больше не видит русское слово). Adversarial-ревью (3 линзы → верификация): 10 находок починены — гонка сброса ReportFlow (синхронный reset), reply на закрытом тикете, confetti на жалобе → нейтральный чек, i18n категорий/плейсхолдеров, a11y (role=log, aria-pressed). Build web зелёный, backend не менялся (36 jest), проверено скриншотами (тред + жалоба + заказ не задиспьючен). |
| 2026-07-09 | **G2 «Отмена заказа» реализован** (спека полноты, п.8 ТЗ). Новый переиспользуемый `ConfirmDialog` (центрированный alert: focus-трап/restore, aria-labelledby/describedby, Esc/бэкдроп заблокированы во время запроса, слот ошибки). Две ветки с разными последствиями: **до принятия** (заказ `active`) — на экране откликов «Отменить заказ» → confirm «Все отклики пропадут» → бэкенд `cancel` (ЗВ-07) + **каскад pending-откликов → not_selected** (специалист видит «Не выбран», а не висящее «Вы откликнулись»; +3 jest-теста); **после принятия** (`in_progress`/`awaiting_confirmation`) — «Отмена через поддержку» → спор-тикет → заказ `disputed` (ЗВ-06). Добавлен баннер для `disputed` (был тупиковый экран без таймлайна/действий). Adversarial-ревью (3 линзы → верификация): 13 находок, починены значимые — молчаливые catch → inline-ошибки, инвалидация общего кэша `['order']`/`['my-orders']`, обработка гонки `cancel_after_accept_via_support`, a11y диалога, честность копирайта (каскад). Build web+api зелёный, **36 jest**, проверено скриншотами обеих веток + disputed. |
| 2026-07-09 | **G1 «Ни одной мёртвой кнопки + все состояния» реализован** (спека полноты по ТЗ, [12-built.md](12-built.md)). Оживлена загрузка диплома в анкете специалиста (S-05, грузится после создания профиля); «Пожаловаться» на отзыве и переключение роли из шелла специалиста больше не тупики; убраны мёртвые строки «Безопасность/Приватность» в настройках; информационные уведомления без диплинка — статичные (не кнопки). Inline-валидация создания заказа (S-20: подсветка + текст «какое поле не заполнено»). Фильтр истории заказов (ИЗ-02: Активные/Завершённые/Отменённые + пустые состояния на вкладку). Новый `ErrorState` (icon+текст+retry) и error/loading-состояния на 8 экранах (лента/отклики/баланс/карты/деталь/поддержка/пополнение) — сбой больше не маскируется под «пусто» и не висит вечным скелетоном. i18n-хардкод колоды вынесен в ru+kk. Build зелёный (tsc+vite), проверено скриншотами. Аддитивно — готовый функционал не тронут. |
| 2026-07-08 | **Спринт 3A «Доверие и безопасность» реализован** (роадмап §Спринт 3). Safety-слой на активном заказе (`SafetySheet`: поделиться статусом через Web Share + fallback, SOS·112 `tel:`, «Сообщить о проблеме» → поддержка, доверенные контакты в localStorage). Двухшаговый условный отзыв S-27: звезда → 4–5★ хвалебные теги (публичный отзыв) / 1–3★ приватная форма проблемы → **тикет в поддержку** (публична только звезда, честно к копирайту). Маскирование контактов в тексте отклика (backend `maskContacts` — телефоны ≥10 цифр/ссылки/@ники/мессенджеры → •••; годы/ставки сохраняются). Adversarial код-ревью: 4 бага исправлены (публичность негатив-отзыва, over-masking регекса, дедуп контактов, fallback шеринга). Build зелёный, 33 jest, проверено скриншотами. Отложено: №19 «кто посмотрел заказ» (трекинг просмотров). |
| 2026-07-07 | **Вики синхронизирована с кодом** (правило «расхождение кода и вики — баг»): обновлены `03-data-model` (поля Bid `availability`/`hasMaterials`/`comment`), `04-api` (§3.13 uploads/files + структурный отклик), `06-design-system` (Skeleton-система, StatusTimeline, VerifiedBadge/NewSpecialistBadge, MapView, pull-to-refresh), `05-screens` (реальные карты S-10/S-22 `/client/map`, профиль заказчика, waiting-state, таймлайн, галерея фото), `02-architecture` (UploadsModule), `08-integrations` (публичная раздача фото), `glossary` (+5 терминов). Сверено по коду. |
| 2026-07-07 | **Спринт 2 «Кластер решения о ставке» реализован** (роадмап §Спринт 2). **2A:** скелетон-система (Card/List/Detail/Chat-архетипы) вместо «Загрузка…» на всех экранах; таймлайн статуса заказа (Опубликован→Выбран→В работе→Выполнено→Оценено); карточка исполнителя на активном заказе (фото/рейтинг/«Личность подтверждена») + accept-в-чат; pull-to-refresh централизованно в `Screen`. **2B:** реальные **фото заказа** — `UploadsModule` (POST `uploads/image` с клиентским Canvas-сжатием + GET `files/public/:name` с guard от path-traversal, только публичный бакет), фото-пикер в S-20 (до 5, превью, удаление), галерея на S-12/детали + миниатюра в колоде; seed — демо-PNG + сброс `HiddenOrder` демо-специалистов. 2 adversarial код-ревью (2A: 3 бага исправлены; 2B: security-фокус). Build зелёный, 33 jest, проверено скриншотами. Долг: аватары специалистов → follow-up. |
| 2026-07-07 | **Спринт 1 «Быстрые победы» реализован** (роадмап §Спринт 1): waiting-state с скелетонами вместо «Загрузка…» (S-23), человеческие ролевые статусы («Вы откликнулись»/«Ожидает решения»), бейдж «Личность подтверждена» + честный «Новый специалист» (рейтинг прячется без отзывов), подсказка бюджета по категории + микрокопирайт, единый формат тенге, empty-states с CTA, тумблер «Принимаю заказы», OTP-автозаполнение, сохранение черновика заказа. **№8 структурированный отклик** (миграция `bid_structured`: `availability`/`hasMaterials`/`comment` + DTO + сериализация + UI в RespondSheet и карточке S-23). Прошёл adversarial код-ревью (2 бага исправлены: честный рейтинг гейтился по числу заказов вместо рейтинга; OtpInput затирал цифры при правке ячейки). Build зелёный, 33 jest, проверено скриншотами. Отложено: pull-to-refresh (№10) → Спринт 2 (со скелетон-системой). |
