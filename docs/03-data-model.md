...
erDiagram
    ...
    Bid {
        ...
        string specialistId FK
        int counterPrice "встречная цена заказчика (G6)"
        string status "pending|countered|accepted|declined|not_selected"
    }
    HiddenOrder {
        string specialistId FK
        string orderId FK
...
| `counterPrice` | `Int?` | Встречная цена заказчика (G6, миграция `bid_counteroffer`, статус `countered`). API отдаёт `counter_price`; при закрытии сделки становится финальной ценой (комиссия считается от неё) |
| `commission` | `Int` | Комиссия сервиса, целые тенге; `ORDER_COMMISSION_PCT` (env, по умолчанию 5) от финальной цены; показывается на S-13, **списывается с баланса в момент принятия отклика** (ADR-001) |
...
| `status` | enum, см. §3 | `pending \| countered \| accepted \| declined \| not_selected` |
| `createdAt` | `DateTime` | |
...
| `Bid.status` | `pending \| countered \| accepted \| declined \| not_selected` |
