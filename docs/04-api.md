...
| Метод и путь | Auth / роль | Описание | Трассировка |
|---|---|---|---|
| `POST orders/:id/bids` | JWT, специалист | Отклик: цена (принять цену заказчика или предложить свою). Требования: верификация пройдена (В-06), подписка активна (иначе блок S-17, БП-02/БП-06). Один отклик на заказ от специалиста (уникальность `orderId+specialistId`). В ответе/превью — комиссия и «Вы получите» (ADR-001) | В-06, БП-02, БП-06, S-13 |
| `GET orders/:id/bids` | JWT, заказчик | Отклики на мой заказ: профиль, бейдж «Дипломированный ✓», рейтинг, расстояние, цена | S-23, О-05, ОМ-08 |
...
- **TODO(M3)** — механизм загрузки файлов (multipart напрямую в API vs presigned URL MinIO) для верификации, диплома, фото заказа и вложений тикетов.
