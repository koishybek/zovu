...
1. **M8 — Polish (финал):** сид-данные Алматы (заказы «Установить розетку · 5 000 ₸» и др. + 5–6 специалистов с рейтингами), карта OSM/Leaflet (S-10/S-22), фильтры-UI S-21, отзывы S-33, доводка анимаций/haptics/empty-states по §4, e2e happy-path скрипт, финальное обновление вики и README, команды запуска ≤ 5.

Это последний майлстоун — после него проект соответствует Definition of Done (§12 промпта).
