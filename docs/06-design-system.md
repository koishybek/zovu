...
- **Пополнения POST-M8** (спринты 1/2A, коммиты `41f134a`, `37fa2e6`):
  - **Skeleton-система** (`ui/Skeleton.tsx`) — серый shimmer-блок под реальный layout **вместо спиннера/«Загрузка…»**: базовый `Skeleton` (width/height/radius/circle) + пресеты `SkeletonBidCard` (аватар + строки + пилюля цены), `SkeletonCard` (пилюля статуса + заголовок + адрес + цена), `SkeletonList` (N карточек, default 4), `SkeletonDetail` (заголовок + баннер + строки), `SkeletonChat` (пузыри слева/справа).
  - **StatusTimeline** (`ui/StatusTimeline.tsx`) — таймлайн заказа в стиле Uber/DoorDash: **Опубликован → Выбран → В работе → Выполнено → Оценено** (i18n `timeline.*`) на активном заказе (`features/deal/ActiveOrderScreen`). Терминальные `completed`/`completed_auto` — все шаги done без пульсации; `cancelled`/`disputed` таймлайн не показывают (объясняет отдельный баннер). Индекс шага маппится из статуса заказа ([07-business-rules.md](07-business-rules.md)).
  - **VerifiedBadge** «Личность подтверждена» (В-06: щит + success-тон, сигнал доверия для заказчика) и **NewSpecialistBadge** «Новый специалист» (нейтральный бейдж вместо фейковой 0.0/5.0 без истории) — в `ui/Badge.tsx` рядом с `DiplomaBadge`/`CountBadge`.
- **MapView** (`src/components/MapView.tsx`, НЕ в `ui/`) — **реальная карта OSM на Leaflet** (заглушек S-10/S-22 больше нет, коммиты `2d67b46`/`c81d8cf`): маркеры-пилюли `divIcon` на брендовых цветах (variant `price` `#4C6FFF` · `me` `#16A34A` · `specialist` `#141824`), клик → `onClick`, авто-`fitBounds`, fallback-центр `ALMATY_FALLBACK`. Используется в `features/orders/OrdersMap.tsx`, `SpecialistMapScreen` (`/sp/map`), `ClientMapScreen` (`/client/map`).
- **Pull-to-refresh** централизован в `ui/Screen.tsx` (проп `onRefresh`): тач-жест у верха списка, порог 64px, деградирует тихо при ошибке обновления. Заменяет правило «pull-to-refresh — нативный web» из §7 конкретной реализацией.
