...
| S-22 | Карта заказчика | `/client/orders/:id/map` | push |
...
- **Содержимое:** карта (Leaflet + OSM-тайлы, см. [08-integrations.md](08-integrations.md)) с маркерами активных заказов **в категориях специалиста**; кластеризация при зуме.
- **Правила:** показываются только заказы, под фильтры которых специалист подходит (Ф-07). API: `GET orders/nearby-map`.
- **Переходы:** тап по маркеру → мини-карточка заказа → тап → S-12.
...
- **Содержимое:** чип категории, название, цена крупно, расстояние, адрес, фото, секция «Описание», строки «Когда удобно» / «Длительность», CTA «Откликнуться».
...
Канвас: `design/canvases/client.dc.html`. Shell-таббар: **Заказы / Отклики / Чаты / Профиль**.
...
- **Содержимое:** заголовок «Создание заказа»; категория\* (bottom sheet справочника + «Предложить свою»), «Что надо сделать»\*, «Фото (до 5)» (сжатие перед загрузкой на клиенте — НФ-08, через Canvas/`browser-image-compression`), «Бюджет (₸)»\*, «Адрес» (автоопределение по Geolocation API + ручной ввод), CTA «Опубликовать». Иконка ⓘ → фильтры (S-21).
- **Правила:** обязательные поля со звёздочкой; ≤ 5 фото. Публикация → заказ `active` (state machine в [07-business-rules.md](07-business-rules.md)). API: `POST orders`.
...
### S-22 Карта заказчика — `/client/orders/:id/map`
- **Содержимое:** свой заказ по центру карты, маркеры подходящих под фильтры специалистов; снизу карточка «Ваш заказ: <название> · <бюджет> · до N км».
- **Правила:** маркеры — только специалисты, проходящие фильтры заказа (Ф-07).
- **Переходы:** к откликам → S-23; назад → «Мои заказы».

### S-23 Отклики (список) — `/client/orders/:id/bids` (таб «Отклики», рут `/client/bids`)
- **Содержимое:** заголовок «Получено N откликов»; карточки: фото, имя, бейдж ✓ (дипломированный, ДС-\*), ★рейтинг (кол-во отзывов), расстояние, «Цена N ₸».
- **Правила:** рейтинг и отзывы специалиста видны заказчику; скрытые отзывы исключены (О-05, ОМ-08). Таб-рут `/client/bids`: при одном активном заказе — сразу S-23 этого заказа; при нескольких — группировка по заказам (кейс в источниках не описан — TODO(M4), см. openQuestions). API: `GET orders/:id/bids`.
- **Переходы:** тап по карточке → S-24.
...
- **Содержимое:** статус-баннер «В работе · Специалист уже приступил к выполнению.», данные заказа, блок «Исполнитель» (тап → профиль/отзывы специалиста), строка «Договорились <дата, время>», CTA «Завершить заказ».
...
- **Переходы:** «Завершить заказ» → S-26 (тот же роут, новое состояние); чат → S-30; «Исполнитель» → S-33.
