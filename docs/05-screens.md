...
- **Содержимое:** табы «Все / Активные / Завершённые» (как на макете «Статусы откликов», G4 п.12): «Активные» = отклик ещё в игре (pending/countered — ждёт решения, либо принятый на живом заказе in_progress/awaiting_confirmation/disputed — во время спора чат остаётся доступен); «Завершённые» = not_selected/declined и принятый на завершённом заказе; «Все» = без фильтра. Карточки со статус-чипами «Ожидание ответа» (warning), «Встречная цена» (review), «Принят» (success), «Не выбран» (inkSecondary `#6B7280` на divider `#F0F1F4`) — палитра чипов в [06-design-system.md](06-design-system.md). Карточка нажимаема только у активной сделки (ведёт в чат); прочие — статичные. Контр-отклик (countered) показывает «Заказчик предложил X» + «Принять X» / «Предложить другую» (G6).
...
| Каскад «Не выбран» (not_selected) | специалист | S-14 `/sp/bids?tab=done` |
